     ...
     function customerMarketing(){
         ...
         if (isset($_SESSION['id'])) {
           foreach ($_SESSION['basket'] as $id => $value) {
              ...
              //$basketString = "$productName , ";
              
              $basketString = "$basketString , $productName ";
            }    
            //echo $basketString;
            global $customerId;
            //$id2 = 1;
            $sql = $conn->prepare("UPDATE `customer` SET `Items_Bought` = '$basketString' WHERE `Customer_ID` = '$customerId'");
            $sql->execute();
        } 
    }   
